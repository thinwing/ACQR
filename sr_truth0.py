...
for i in range(config.trial):
    ...
    for gamma in config.gamma:
        for method in config.methods:
            ...
            sr_path = data_path + '/base/CQR0/same_range.npz'
            ground_result = np.load(sr_path)
            grd_truth_CQR = ground_result['func_est']
            
            func_path = data_path + '/online/pinball_moreau/' + '\u03b3=' + str(gamma) + '/CQR0/' + str(method) + '.npz'
            ...
            save_path = 'truth/linear_expansion/sparse/outlier_rate=' + str(outlier_rate) + '/Iter=' + str(config.Iter) + '/alpha=0.95/trial=' + str(i+1) + '/online/pinball_moreau/' + '\u03b3=' + str(gamma) + '/OCQKR0'
