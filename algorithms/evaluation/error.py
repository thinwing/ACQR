...
def range_error(func_est, gt, Iter, method):
    ...
    if range_func_est.ndim == 1:
        if method['processing'] == 'online':
            range_array = np.array([range_func_est])
            range_func_est_ave = np.sum(range_func_est) / len(range_array[0])

            coverage_db_temp = np.sum((range_func_est - range_gt.T) ** 2) / np.sum(range_gt ** 2, axis=0)  

            coverage_db_high = np.sum((func_est[1] - gt[1].T) ** 2) / np.sum(gt[1] ** 2, axis=0)        
            coverage_db_low = np.sum((func_est[0] - gt[0].T) ** 2) / np.sum(gt[0] ** 2, axis=0)
