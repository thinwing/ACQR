...
class online_learning(base_learning):
    ...
    def learning(self, loss):
        ...
        if loss['loss'] == 'pmc_online':
            ...
            self.input_e, self.input_f, self.input_g = np.array_split(self.input_c, 3, 0)
            self.input_h = np.vstack((self.input_d, self.input_e))
            ...
            self.output_e, self.output_f, self.output_g = np.array_split(self.output_c, 3, 0)
            self.output_h = np.vstack((self.input_d, self.input_e))
            ...
            self.input_tr = self.input_h
            self.output_tr = self.output_h
            self.input_ca = self.input_f
            self.output_ca = self.output_f
            self.input_te = self.input_g
            self.output_te = self.output_g
...
class online_learning_hi(base_learning_hi):
    ...
    def learning(self, loss):
        ...
        if loss['loss'] == 'pmc_online':
            ...
            total_length = config.Iter
            part1_length = str(total_length*0.85)
            part2_length = str(total_length*0.075)
            part3_length = str(total_length*0.075)
            # 配列を分割
            self.part1 = self.input_test[:part1_length]
            self.part2 = self.input_test[part1_length:part1_length + part2_length]
            self.part3 = self.input_test[part1_length + part2_length:]
            self.part4 = self.output_test[:part1_length]
            self.part5 = self.output_test[part1_length:part1_length + part2_length]
            self.part6 = self.output_test[part1_length + part2_length:]
            ...
            #self.input_ca = self.input_g[calpoints, :]
            #self.output_ca = self.output_g[calpoints]
            self.input_tr = self.part1
            self.output_tr = self.part4
            self.input_ca = self.part2
            self.output_ca = self.part5
            self.input_te = self.part3
            self.output_te = self.part6
            self.Iter_tr = int(len(self.input_tr))
            ol_tr = eval(self.method['method'])(input=self.input_tr, dict_band=self.method['dict_band'])        
            ol_tr.dict_define(self.method['variable'])
            self.train_vector = ol_tr.kernel_vector(self.input_tr)
