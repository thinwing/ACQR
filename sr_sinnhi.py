    ...
    for outlier_type in config.outlier_types:
        ...
        for method in config.methods:
            for i in range(config.trial):
                ...
                grd_path = 'result/text/' + 'dim=' + str(config.input_dim) + '/' + str(noise_type) + '/' + str(outlier_type) + '/outlier_rate=' + str(outlier_rate) + '/Iter=' + str(config.Iter) + '/alpha=0.95/hi/trial=' + str(i+1) + '/base'+ '/same_range.npz'    
                grd = np.load(grd_path)
                grd_truth = grd['func_est']
                for gamma in config.gamma:
                    func_path = 'result/text/' + 'dim=' + str(config.input_dim) + '/' + str(noise_type) + '/' + str(outlier_type) + '/outlier_rate=' + str(outlier_rate) + '/Iter=' + str(config.Iter_CQR) + '/alpha=0.95/hi/trial=' + str(i+1) +  '/online/pinball_moreau/\u03b3=' + str(gamma)  + '/CQR'+ '/' + str(method) + '.npz'
                    ...
                    save_path = 'result/text/' + 'dim=' + str(config.input_dim) + '/' + str(noise_type) + '/' + str(outlier_type) + '/outlier_rate=' + str(outlier_rate) + '/Iter=' + str(config.Iter_CQR) + '/alpha=0.95/hi/trial=' + str(i+1) +  '/online/pinball_moreau/\u03b3=' + str(gamma)  + '/CQR' 
