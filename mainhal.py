                    ...
                    for index_alpha, alpha in enumerate(alpha_all):
                        ...
                        for i in range(config.trial):
                            ...
                            if eval('address.' + str(method))['processing'] == 'batch':
                                ...
                                learn = optimize.online_learning_hal(observation=observation, noise=noise, data=data, alpha=alpha, method=eval('address.' + str(method)), trial=i+1, outlier_rate=outlier_rate)
