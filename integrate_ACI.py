...
def data_integratehi(data_path, method, loss, gamma, trial=config.trial):
    cov_temp = np.zeros(trial)
    cov_db_temp = np.zeros([trial, 3])
    cov_db_interval = np.zeros([3, 2])
    ...
    cov_interval = st.t.interval(alpha_rel, deg_free, loc=cov_ave, scale=cov_scale)
    # save
    save_path = data_path + '/hi/online/pinball_moreau/ACI.npz'
    save_path_add = '/hi/online/pinball_moreau/'
    print(save_path)
    mkdir(data_path+save_path_add, exist_ok=True)
    np.savez_compressed(save_path, coverage=coverage, coverage_all=coverage_all, coverage_interval=cov_interval, coverage_db=10 * np.log10(coverage_db), coverage_db_interval=(10 * np.log10(cov_db_interval)), range_ave=range_ave)

def data_integratelo(data_path, method, loss, gamma, trial=config.trial):
    cov_temp = np.zeros(trial)
    cov_db_temp = np.zeros([trial, 3])
    cov_db_interval = np.zeros([3, 2])
    ...
    cov_interval = st.t.interval(alpha_rel, deg_free, loc=cov_ave, scale=cov_scale)
    # save
    save_path = data_path + '/lo/online/pinball_moreau/ACI.npz'
    save_path_add = '/lo/online/pinball_moreau/'
    print(save_path)
    mkdir(data_path+save_path_add, exist_ok=True)
    np.savez_compressed(save_path, coverage=coverage, coverage_all=coverage_all, coverage_interval=cov_interval, coverage_db=10 * np.log10(coverage_db), coverage_db_interval=(10 * np.log10(cov_db_interval)), range_ave=range_ave)

def data_integratehal(data_path, method, loss, gamma, trial=config.trial):
    cov_temp = np.zeros(trial)
    cov_db_temp = np.zeros([trial, 3])
    cov_db_interval = np.zeros([3, 2])
    ...
    cov_interval = st.t.interval(alpha_rel, deg_free, loc=cov_ave, scale=cov_scale)
    # save
    save_path = data_path + '/hal/online/pinball_moreau/ACI.npz'
    save_path_add = '/hal/online/pinball_moreau/'
    print(save_path)
    mkdir(data_path+save_path_add, exist_ok=True)
    np.savez_compressed(save_path, coverage=coverage, coverage_all=coverage_all, coverage_interval=cov_interval, coverage_db=10 * np.log10(coverage_db), coverage_db_interval=(10 * np.log10(cov_db_interval)), range_ave=range_ave)
