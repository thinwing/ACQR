                ...
                for index_alpha, alpha in enumerate(alpha_all):
                    ...
                    for i in range(config.trial):
                        ...
                        if eval('address.' + str(method))['processing'] == 'batch':
                            ...
                            learn = optimize_CQR100.online_learning(observation=observation, noise=noise, data=data, alpha=alpha, method=eval('address.' + str(method)), trial=i+1, outlier_rate=outlier_rate)
                            grd_truth = optimize_CQR100.gtCQR(data_path=learn.data_path, observation=observation, noise=noise, data=data, alpha=alpha)
                            learn.pre_learning()
                    ...
                    if eval('address.' + str(method))['processing'] == 'batch':
                        ...
                        for loss_temp in config.losses:
                            if str(loss_temp) == 'pinball':
                                ...
                                for gamma in config.gamma:
                                    integrate_CQR100(data_path=learn.data_path_temp, method=str(method), loss=loss_temp, gamma=gamma, trial=config.trial)
