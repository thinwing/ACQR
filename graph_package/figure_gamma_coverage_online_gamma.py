...
def comp_proposed_coverage(data_path, loss_list, method_list): 
    ...
    for _, range_alpha_temp in enumerate(range_alpha):
        ...
        for index_method, method in enumerate(method_list_flatten):                
            for _, item in enumerate(list_flatten):
                if item == 'pinball':
                    ...
                    # ax.plot(iteration, coverage, label=eval('grp.' + str(item))['loss_name'] + eval('grp.' + str(method))['fig_name2'], linewidth=grp.linewidth)
                    ax.plot(iteration, coverage, label=eval('grp.' + str(item))['loss_name'], linewidth=grp.linewidth)
                else:
                    for _, g in enumerate(config.gamma):
                        data_path_detail, _ = get_path(data_path=data_path_alpha, method=method, loss=item, gamma=g)
                               
                        method_temp = np.load(data_path_detail)
                        coverage_temp = method_temp['coverage']
                        coverage = coverage_temp[1] - coverage_temp[0]
                        
                        # ax.plot(iteration, coverage, label=eval(r'' + 'grp.' + str(item))['loss_name'] + eval('grp.' + str(method))['fig_name2'] + ', $\gamma = $' + str(np.round(g, 2)), linewidth=grp.linewidth)
                        ax.plot(iteration, coverage, label=eval(r'' + 'grp.' + str(item))['loss_name'] + ', $\gamma = $' + str(np.round(g, 2)), linewidth=grp.linewidth)
        ...
        save_name = save_path_alpha + '/coverage_gamma_gamma.pdf'
...
#                         ax.plot(iteration, coverage_db, label=eval('grp.' + str(item))['loss_name'] + eval('grp.' + str(method))['fig_name2'] + ', \gamma = ' + str(g), linewidth=grp.linewidth)
...
#         save_path_alpha = data_path_alpha.replace('text','graph')
#         mkdir(save_path_alpha, exist_ok=True)
#         save_name = save_path_alpha + '/coverage_gamma_db_gamma.pdf'
#         plt.savefig(save_name, bbox_inches='tight')        
#         plt.clf()
#         plt.close()
