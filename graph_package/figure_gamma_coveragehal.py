...
def gamma_coveragehal(data_path, loss_list, method='single_kernel', alpha=config.alpha_range):
    gamma = config.gamma
    ...
    for _, item in enumerate(list_flatten):
        if item == 'pinball':
            ...
            data_path_detail = data_path_detail.replace('0.95/', '0.95/hal/')
...
# comparative ACI 
        for index_gamma, gamma_temp in enumerate(gamma):
            # data load
            data_path_detail = 'result/text/dim=1/linear_expansion/sparse/=0.05/Iter=3000/alpha=0.95/hal/online/pinball_moreau/ACI.npz'
    ...
    save_name = save_path + '/coverage_gammahal.pdf'
...
def gamma_errorhal(data_path, loss_list, method='single_kernel', alpha=config.alpha_range):
    gamma = config.gamma
...
# proposed OCQKR
        ...
        save_path = data_path.replace('text', 'graph')
        mkdir(save_path, exist_ok=True)
        save_name = save_path + '/hal' + grp.title_coverage_db_gamma[i]
        plt.savefig(save_name, bbox_inches='tight')
        plt.clf()
        plt.close()
