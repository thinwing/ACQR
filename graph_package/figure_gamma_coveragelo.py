...
def gamma_coveragelo(data_path, loss_list, method='single_kernel', alpha=config.alpha_range):
    gamma = config.gamma
    ...
    for _, item in enumerate(list_flatten):
        if item == 'pinball':
            ...
            data_path_detail = data_path_detail.replace('0.95/', '0.95/lo/')
...
# comparative ACI 
        for index_gamma, gamma_temp in enumerate(gamma):
            # data load
            data_path_detail = 'result/text/dim=1/linear_expansion/sparse/=0.05/Iter=3000/alpha=0.95/lo/online/pinball_moreau/ACI.npz'
    ...
    # proposed OCQKR               
        for index_gamma, gamma_temp in enumerate(gamma):
            # data load
            data_path_detail = 'result/text/dim=1/linear_expansion/sparse/outlier_rate=0.05/Iter=1000/alpha=0.95/lo/online/pinball_moreau/' + '/\u03b3=' + str(gamma_temp) + '/CQR/single_kernel.npz'
    ...
    # proposed OCQMKR               
        for index_gamma, gamma_temp in enumerate(gamma):
            # data load
            data_path_detail = 'result/text/dim=1/linear_expansion/sparse/outlier_rate=0.05/Iter=1000/alpha=0.95/lo/online/pinball_moreau/' + '/\u03b3=' + str(gamma_temp) + '/CQR/multi_kernel.npz'
            #'data_path_detail, _ = get_path_CQR(data_path=data_path_alpha, method=method, loss=item, gamma=gamma_temp)'
    ...
    save_name = save_path + '/coverage_gammalo.pdf'
...
def gamma_errorlo(data_path, loss_list, method='single_kernel', alpha=config.alpha_range):
    gamma = config.gamma
...
# proposed OCQKR
        ...
        save_path = data_path.replace('text', 'graph')
        mkdir(save_path, exist_ok=True)
        save_name = save_path + '/lo' + grp.title_coverage_db_gamma[i]
        plt.savefig(save_name, bbox_inches='tight')
        plt.clf()
        plt.close()
