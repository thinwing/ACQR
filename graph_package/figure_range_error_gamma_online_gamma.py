...
def comp_proposed_coverage_db(data_path, loss_list, method_list):
    ...
    for i in range(3):
        for _, range_alpha_temp in enumerate(range_alpha):
            ...
            for index_method, method in enumerate(method_list_flatten):
                for _, item in enumerate(list_flatten):
                    if item == 'pinball':
                        ...
                        ax.plot(iteration, coverage_db, label=eval('grp.' + str(item))['loss_name'], color=eval('grp.' + str(item))['color'][index_method], linewidth=grp.linewidth)
                        ...
                        for _, g in enumerate(config.gamma):
                            ...
                            # ax.plot(iteration, coverage_db, label=r'' + eval('grp.' + str(item))['loss_name'] + eval('grp.' + str(method))['fig_name2'] + ', $\gamma = $' + str(np.round(g, 2)), linewidth=grp.linewidth)
                            ax.plot(iteration, coverage_db, label=r'' + eval('grp.' + str(item))['loss_name'] + ', $\gamma = $' + str(np.round(g, 2)), linewidth=grp.linewidth)
            ...
            save_name = save_path_alpha + '/' + 'gamma_gamma_' + str(grp.title_coverage_db[i])
