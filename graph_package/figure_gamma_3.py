...
def gamma_coverage3(data_path, loss_list, method='single_kernel', alpha=config.alpha_range):
    gamma = config.gamma
    ...
    # proposed OCQKR               
        ...
        ax1.plot(gamma, result_coverage, label='2.5-97.5', color='red', linewidth=grp.linewidth, marker=eval('grp.' + str(item))['marker'], markersize=grp.marker_size)

        for index_gamma, gamma_temp in enumerate(gamma):
            # data load
            data_path_detail = 'result/text/dim=1/linear_expansion/sparse/outlier_rate=0.04/Iter=1000/alpha=0.95/online/pinball_moreau/' + '/\u03b3=' + str(gamma_temp) + '/CQR0/single_kernel.npz'
            #'data_path_detail, _ = get_path_CQR(data_path=data_path_alpha, method=method, loss=item, gamma=gamma_temp)'
            ...
            result_coverage[index_gamma] = coverage[-1]
            print(result_coverage[index_gamma])
        ax1.plot(gamma, result_coverage, label='0-95', color='blue', linewidth=grp.linewidth, marker='s', markersize=grp.marker_size)

        for index_gamma, gamma_temp in enumerate(gamma):
            # data load
            data_path_detail = 'result/text/dim=1/linear_expansion/sparse/outlier_rate=0.04/Iter=1000/alpha=0.95/online/pinball_moreau/' + '/\u03b3=' + str(gamma_temp) + '/CQR100/single_kernel.npz'
            #'data_path_detail, _ = get_path_CQR(data_path=data_path_alpha, method=method, loss=item, gamma=gamma_temp)'
            ...
            result_coverage[index_gamma] = coverage[-1]
            print(result_coverage[index_gamma])
        ax1.plot(gamma, result_coverage, label='5-100', color='green', linewidth=grp.linewidth, marker='o', markersize=grp.marker_size)
    #ここまで追加パッチ
    ...
    save_path = 'New/submit'
    mkdir(save_path, exist_ok=True)
    save_name = save_path + '/coverage_gamma3.pdf'
...
def gamma_error3(data_path, loss_list, method='single_kernel', alpha=config.alpha_range):
    gamma = config.gamma
    ...
    for i in range(3):
        ...
        for _, item in enumerate(list_flatten):
            if item == 'pinball':
                ...
                for index_gamma, gamma_temp in enumerate(gamma):
                    # data load
                    data_path_detail = 'truth/linear_expansion/sparse/outlier_rate=0.04/Iter=3000/alpha=0.95/online/pinball_moreau/γ=' + str(gamma_temp) + '/OCQKR/single_kernel.npz'
...
# proposed OCQMKR
            ...
            for index_gamma, gamma_temp in enumerate(gamma):
                # data load
                data_path_detail = 'truth/linear_expansion/sparse/outlier_rate=0.04/Iter=3000/alpha=0.95/online/pinball_moreau/γ=' + str(gamma_temp) + '/OCQKR0/single_kernel.npz'
            ...
            for index_gamma, gamma_temp in enumerate(gamma):
                # data load
                data_path_detail = 'truth/linear_expansion/sparse/outlier_rate=0.04/Iter=3000/alpha=0.95/online/pinball_moreau/γ=' + str(gamma_temp) + '/OCQKR100/single_kernel.npz'
